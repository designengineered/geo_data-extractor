...
# Script to extract basic metadata (filename, date, GPS) from PNG and JPEG images
# Usage: ./extract_image_metadata.sh [input_directory]
...
extract_metadata() {
    ...
    if command -v exiftool >/dev/null 2>&1; then
        # Extract and format date/time (just the first one found)
        local datetime=$(exiftool -s -s -s -DateTimeOriginal -d "%Y-%m-%d %H:%M:%S" "$input_file" 2>/dev/null)
        if [ ! -z "$datetime" ]; then
...
LOG_FILE="$REPORTS_DIR/image_metadata_$(date '+%Y%m%d_%H%M%S').txt"

# Create temporary file for the main content
TEMP_FILE=$(mktemp)

# Initialize temp file with header
echo "Image File Metadata Summary" > "$TEMP_FILE"
...
# Process all PNG and JPEG files
